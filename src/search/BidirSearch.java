...
/**
 * Bidirectional Search algorithm based on the VLDB 2005 paper titled "Bidirectional Expansion For Keyword Search 
 * on Graph Databases" by V. Kacholia et. al.
 * 
...
public class BidirSearch implements Iterator<MyTreeResult>{
  ...
  ArrayList<HashMap<Node,Float>> activationValues; // HashMap of activation values of each node per keyword indexed by the list. The list length=#keywords.
  HashMap<Node,Integer> depthVals; // The depth at which a node was discovered.
  ArrayList<HashMap<Node,Node>> SP; // HashMap of next node to follow from each node to reach a keyword node indexed by the list. The list length=#keywords.
  ArrayList<HashMap<Node,Integer>> distValues; // HashMap of shortest distance from each node to reach a keyword node indexed by the list. The list length=#keywords.
  ...
  public List<List<Node>> getKeywordSet(){
    ...
    for (String entity : entities){
      ...
      if(SelectivePathComputer.nameToNum.containsKey(entity)){
        Node val = new Node(SelectivePathComputer.nameToNum.get(entity));
        listOfNodes.add(val);
  ...
  public void init() {
    ...
    for(int i=0;i<S.size();i++){
      ...
      for(int j=0;j<temp.size();j++){
        ...
        /*Initialize dist vals. Is getting done in getKeywordSet().
        for(int k=0;k<S.size();k++) {
          HashMap<Node,Integer> distVals = new HashMap<Node,Integer>();
          if(k==i)
            distVals.put(temp.get(j), 0);
          else
   ...
   * @param depth The depth at which it was discovered. If it is -1, then it is being inserted
   * by the incoming operator and need not be updated now.
   */
  ...
  private Float computeActivation(Node u){//, NodeActVals va,int direction) {
    //Compute activation of u from va where direction =0 imples edge u->v and 1 imples v->u
  ...
  /**
   * Detects if the egde u-->v constitutes a cycle. It checks if {@link Node} v exists in the
   * ancestors of {@Node} u recursively.
   * @param u Source node
   * @param v Target node
  ...
  private boolean exploreEdge(Triple q, int inOrOut) {
    ...
    for(int i=0;i<querySplit.length;i++){
      ...
      if(tmp.get(v)!=null){//TODO is null for outgoing iterator almost always, as the target node is still unexplored.

        // If no entry exists for u, set its value to Integer.MAX_VALUE to be checked and set to correct val in the following check.
        if(tmm==null){
  ...
  private void Attach(Node u, int i) {
    //Paper says we need to update priority of u in Qin. I feel it is not required as we have not updated activation values yet.

    //propagating change in cost dist (v,k) to all ancestors in best first manner. propagateChangeInDistance(u,i);
   ...
   */
  private void activate(Node u, int i, Float oldit) { // Note that a change in activation values may change the priority queue
    ...
    //Propagate change in Activation value to all reached ancestors in best first manner.
    ArrayList<Node> ancestors =this.reachedAncestors.get(u);
    if(ancestors==null || ancestors.size()==0)
      return;
    for(int c=0;c<ancestors.size();c++){
  ...
  private Float getActivation(Node u, Node v, int i, int inOrOut) {
    ...
    return (new Float(0.5))*(toCheckVal/size); //0.5 attenuation factor, rest 0.5 divided in inverse ratio of the number of edges.
  }
  ...
  private void emitResult(Node n) {
    ...
    Debug.println("Exiting emitResult()");
  }
  ...
  /**
   * Checks if a result is duplicate. Required to remove duplicate answers with different roots. Refer section 4.2.3 of the paper.
  ...
  public boolean hasNext() {
    ...
    while(Qin.size()!=0 || Qout.size()!=0){
      System.out.println("Either Qin or Qout has a node.");
      if(is_emitted)
        break;
      ...
      if(maxQinNA!=null){
        ...
        System.out.println("A node exists in Qin:"+maxQin+" with activation:"+QinA);
      }
      if(maxQoutNA!=null){
        maxQout=maxQoutNA.getNode();
        QoutA=maxQoutNA.getActivationval();
        System.out.println("A node exists in Qout:"+maxQout+" with activation:"+QoutA);
      }
      if(QinA>QoutA){ //Qin has higher activation.
        ...
        System.out.println("Qin has higher activation:"+maxQin);
        ...
        if(depthVals.get(maxQin)<=DMAX){ //if depth(v)<Dmax then
          ...
          //  EXPLOREEDGE(u,v)
          System.out.println("Exploring incoming edges into maxQin.");
          Iterator<Triple> itt=dg.find(Node.ANY, ANY_PREDICATE, maxQin);
          boolean foundEdges = false;
          while(itt.hasNext()){
            Triple q=itt.next();
            boolean val=exploreEdge(q,0);
            if(!val) // Why is the check required? exploreEdge(q) returns true only when the edge does not lead to a cycle.
              continue;
            foundEdges = true;
            Node u=new Node(q.getSubject());
          ...
          if(!Xout.contains(maxQinNA.getNode())){
            System.out.println("Adding node to Qout:"+maxQinNA.getNode().toString());
            // Compute remaining activation with the node being explored.
            if(foundEdges)
              maxQinNA.setActivationVal(maxQinNA.getActivationval()/2);
      ...
      { 
        ...
        System.out.println("Qout has higher activation:"+maxQout);

        //if IS_COMPLETE(u) then EMIT(u)
        if (isComplete(maxQout))
          emitResult(maxQout);
        System.out.println("Node being considered in Qout:"+maxQout.toString());
        if(depthVals.get(maxQout)<=DMAX){

          try{
            System.out.println("Exploring outgoing edges from maxQout node...");
            ...
            while(itt.hasNext()){
              ...
              if(!val) // Why is the check required? exploreEdge(q) returns true only when the edge does not lead to a cycle.
                continue;
              System.out.println("Returned after exploring edge.");
            ...
            System.out.println("In QoutA>>>>The size of Qout:"+Qout.size()+" Size of Qin:"+Qin.size());
          }catch(Exception e){
